...
/**
 * @File    :   1559.二维网格图中探测环.go
 * @Time    :   2020/08/28 22:09:22
 * @Author  :   wylu
 * @Version :   1.0
...
func containsCycle(grid [][]byte) bool {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < m; j++ {
			if !visit[i][j] && dfs1559(grid, i, j, -1, -1, &visit) {
				return true
			}
		}
	}
...
func dfs1559(grid [][]byte, x, y, px, py int, visit *[][]bool) bool {
	n, m := len(grid), len(grid[0])
	(*visit)[x][y] = true
	d := [5]int{0, 1, 0, -1, 0}
	for i := 0; i < 4; i++ {
		...
		if 0 <= nx && nx < n && 0 <= ny && ny < m && grid[nx][ny] == grid[x][y] {
			...
			if dfs1559(grid, nx, ny, x, y, visit) {
				return true
			}
		}
	}
