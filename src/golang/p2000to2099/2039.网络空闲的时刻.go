 ...
 * [2039] 网络空闲的时刻
 *
 * https://leetcode-cn.com/problems/the-time-when-the-network-becomes-idle/description/
 *
 * algorithms
 * Medium (47.65%)
 * Likes:    28
 ...
 * - 数据服务器 2 还没收到任何回复。距离上次发出信息过去了 1 秒（1 == patience[2] = 1），所以它重发一条信息（用 2B
 * 表示）。
 *
 * 2 秒时，
...
/**
 * @File    :   2039.网络空闲的时刻.go
 * @Time    :   2022/03/20 09:41:40
 * @Author  :   wylu
 * @Version :   1.0
...
func networkBecomesIdle(edges [][]int, patience []int) int {
	...
	ans := math.MinInt
	dists, seen := make([]int, n), make([]bool, n)
	que := []int{0}
	seen[0] = true
	for len(que) > 0 {
		u := que[0]
		...
		for _, v := range graph[u] {
			if !seen[v] {
				...
				que = append(que, v)
				seen[v] = true
				cur := (dists[v]*2-1)/patience[v]*patience[v] + dists[v]*2 + 1
				if cur > ans {
					ans = cur
				}
			}
...
// 	ans := math.MinInt
// 	for i := 1; i < n; i++ {
// 		cur := (dists[i]*2-1)/patience[i]*patience[i] + dists[i]*2 + 1
// 		if cur > ans {
// 			ans = cur
// 		}
// 	}
