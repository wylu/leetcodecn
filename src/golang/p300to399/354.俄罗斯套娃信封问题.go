...
/**
 * @File    :   354.俄罗斯套娃信封问题.go
 * @Time    :   2021/03/06 22:22:49
 * @Author  :   wylu
 * @Version :   1.0
...
func maxEnvelopes(envelopes [][]int) int {
	...
	lowerBound := func(left, right, target int) int {
		for left < right {
			mid := (left + right) / 2
			if f[mid] < target {
				left = mid + 1
	...
	for i := 1; i < n; i++ {
		m := len(f)
		if envelopes[i][1] > f[m-1] {
			f = append(f, envelopes[i][1])
		} else {
			f[lowerBound(0, m-1, envelopes[i][1])] = envelopes[i][1]
		}
	}
