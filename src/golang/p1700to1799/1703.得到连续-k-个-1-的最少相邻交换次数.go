...
/**
 * @File    :   1703.得到连续-k-个-1-的最少相邻交换次数.go
 * @Time    :   2021/01/02 18:29:34
 * @Author  :   wylu
 * @Version :   1.0
...
func minMoves(nums []int, k int) int {
	...
	for i := 0; i+k-1 < m; i++ {
		j := i + k - 1
		mid := (i + j) >> 1
		left := int64(a[mid]*(mid-i) - (ps[mid] - ps[i]))
		right := int64(ps[j+1] - ps[mid+1] - (j-mid)*a[mid])
