...
/**
 * @File    :   1723.完成所有工作的最短时间.go
 * @Time    :   2021/05/11 16:45:08
 * @Author  :   wylu
 * @Version :   1.0
...
func minimumTimeRequired(jobs []int, k int) int {
	...
	var dfs func(workloads []int, limit, cur int) bool
	dfs = func(workloads []int, limit, cur int) bool {
		if cur == n {
			return true
		}

		for i := 0; i < k; i++ {
			if workloads[i]+jobs[cur] <= limit {
				workloads[i] += jobs[cur]
				if dfs(workloads, limit, cur+1) {
					return true
				}
				workloads[i] -= jobs[cur]
			}

			if workloads[i] == 0 || workloads[i]+jobs[cur] == limit {
				break
			}
		}
		return false
