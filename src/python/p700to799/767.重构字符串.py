...
class Solution:
    def reorganizeString(self, S: str) -> str:
        ...
        for ch in S:
            cnts[ch] += 1
            maxCnt = max(maxCnt, cnts[ch])

        if maxCnt > (n + 1) // 2:
            return ''

        ans = []
        pq = [(-v, k) for k, v in cnts.items()]
        heapq.heapify(pq)

        while len(pq) > 1:
            _, c1 = heapq.heappop(pq)
            _, c2 = heapq.heappop(pq)
            ans.append(c1)
            ans.append(c2)
            cnts[c1] -= 1
            cnts[c2] -= 1
            ...
            if cnts[c1] > 0:
                heapq.heappush(pq, (-cnts[c1], c1))
            if cnts[c2] > 0:
                heapq.heappush(pq, (-cnts[c2], c2))
...
if __name__ == "__main__":
    solu = Solution()
    print(solu.reorganizeString("aab"))
    print(solu.reorganizeString("aaab"))
