...
class DiningPhilosophers:
    ...
    def wantsToEat(
            self,
            philosopher: int,
            pickLeftFork: 'Callable[[], None]',  # noqa F821
            pickRightFork: 'Callable[[], None]',  # noqa F821
            eat: 'Callable[[], None]',  # noqa F821
            putLeftFork: 'Callable[[], None]',  # noqa F821
            putRightFork: 'Callable[[], None]') -> None:  # noqa F821
        L, R = philosopher, (philosopher + 1) % 5
        self.sema.acquire()
        self.locks[L].acquire()
        self.locks[R].acquire()
        pickLeftFork()
        pickRightFork()
        eat()
        putRightFork()
        putLeftFork()
        self.locks[R].release()
