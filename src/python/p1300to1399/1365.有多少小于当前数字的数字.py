...
class Solution:
    def smallerNumbersThanCurrent(self, nums: List[int]) -> List[int]:
        ...
        ans = [0] * n
        for i in range(n):
            if pre == -1 or data[i][0] != data[i - 1][0]:
                pre = i
            ans[data[i][1]] = pre
...
if __name__ == "__main__":
    solu = Solution()
    print(solu.smallerNumbersThanCurrent([8, 1, 2, 2, 3]))
    print(solu.smallerNumbersThanCurrent([6, 5, 4, 8]))
    print(solu.smallerNumbersThanCurrent([7, 7, 7, 7]))
