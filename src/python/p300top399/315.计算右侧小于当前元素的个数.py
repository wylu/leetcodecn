    ...
    def countSmaller(self, nums: List[int]) -> List[int]:
        ...
        def merge(left: int, mid: int, right: int) -> None:
            ...
            for x in range(left, k):
                nums[x], indices[x] = tmpNums[x], tmpIndices[x]
...
if __name__ == '__main__':
    solu = Solution()
    print(solu.countSmaller(nums=[5, 2, 6, 1]))
    print(solu.countSmaller(nums=[5, 2, 5, 1]))
    print(solu.countSmaller(nums=[2, 2, 5, 2, 2, 3]))
