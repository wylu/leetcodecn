...
/**
 * @File    :   1705.吃苹果的最大数目.cpp
 * @Time    :   2021/01/01 23:00:48
 * @Author  :   wylu
 * @Version :   1.0
...
public:
    int eatenApples(vector<int>& apples, vector<int>& days) {
        ...
        while (!pq.empty()) ans += eat(pq, n++);
        return ans;
    }

    int eat(priority_queue<PII, vector<PII>, greater<PII>>& pq, int i) {
        int cnt = 0;
        while (!pq.empty() && cnt == 0) {
            auto ad = pq.top();
            pq.pop();
            if (ad.first <= i) continue;
            cnt++, ad.second--;
            if (ad.second && ad.first > i + 1) pq.emplace(ad);
        }
        return cnt;
    }
};
