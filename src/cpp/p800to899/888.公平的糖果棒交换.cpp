...
/**
 * @File    :   888.公平的糖果棒交换.cpp
 * @Time    :   2021/02/01 23:06:27
 * @Author  :   wylu
 * @Version :   1.0
...
public:
    vector<int> fairCandySwap(vector<int>& A, vector<int>& B) {
        ...
        unordered_set<int> exist(B.begin(), B.end());

        int n = A.size();
        for (int i = 0; i < n; i++) {
            int na = sa - A[i], nb = sb + A[i];
            int need = (nb - na) / 2, ress = (nb - na) % 2;
            if (exist.count(need) && ress == 0) return {A[i], need};
        }
