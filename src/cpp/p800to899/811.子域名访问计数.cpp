...
/**
 * @File    :   811.子域名访问计数.cpp
 * @Time    :   2021/04/16 09:46:37
 * @Author  :   wylu
 * @Version :   1.0
...
public:
    vector<string> subdomainVisits(vector<string>& cpdomains) {
        ...
        for (auto& cpd : cpdomains) {
            ...
            int count = stoi(cpd.substr(0, idx));
            string domain = cpd.substr(idx + 1);

            domains[domain] += count;
            for (int i = domain.size() - 1; i >= 0; i--) {
                if (domain[i] == '.') {
                    domains[domain.substr(i + 1)] += count;
                }
            }
        }
...
void prtVector(vector<string> domains) {
    string res = "[";
