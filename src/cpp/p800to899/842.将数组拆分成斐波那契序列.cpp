...
/**
 * @File    :   842.将数组拆分成斐波那契序列.cpp
 * @Time    :   2020/12/08 23:12:57
 * @Author  :   wylu
 * @Version :   1.0
...
public:
    ...
    bool backtrack(int offset, long long f0, long long f1) {
        if (offset == S.length()) {
            return ans.size() > 2;
        }
        ...
        for (int i = offset; i < S.length(); i++) {
            ...
            ans.emplace_back(f2);
            if (backtrack(i + 1, f1, f2)) return true;
            ans.pop_back();
        }
