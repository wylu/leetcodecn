        ...
        while (!que.empty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int i = 1; i <= 6; i++) {
                    ...
                    PII it = getPosition(next_num - 1);
                    int r = it.first, c = it.second;
                    if (board[r][c] != -1) next_num = board[r][c];
                    if (next_num == n * n) return ans;
                    // cout << r << " " << c << " " << next_num << endl;
                    if (!seen.count(next_num)) {
...
int main(int argc, char const* argv[]) {
    ...
    cout << solu.snakesAndLadders(board) << endl;
    return 0;
}
