...
public:
    vector<int> smallestRange(vector<vector<int>>& nums) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            que.push({.val = nums[i][0], .row = i, .col = 0});
        ...
        while (!que.empty()) {
            ...
            que.push({.val = nums[node.row][node.col + 1],
                      .row = node.row,
                      .col = node.col + 1});
        }
...
int main(int argc, char const* argv[]) {
    ...
    vector<int> res = solu.smallestRange(nums);
