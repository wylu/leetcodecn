...
public:
    int videoStitching(vector<vector<int>>& clips, int T) {
        ...
        int pre = 0, start = 0, end = 0, ans = 1;
        for (auto clip : clips) {
            int s = clip[0], e = clip[1];
            if (s > end || end >= T) break;
...
int main(int argc, char const* argv[]) {
    Solution solu;
    vector<vector<int>> clips = {{0, 2}, {4, 6}, {8, 10}, {1, 9}, {1, 5}};
    cout << solu.videoStitching(clips, 10) << endl;
    return 0;
}
